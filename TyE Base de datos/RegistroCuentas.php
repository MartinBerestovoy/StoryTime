...
if(isset($_POST["submit"]))
{
  ...
  if($password == $cpassword)
  {
    ...
    $sql = "SELECT * FROM proyecto2023_emma WHERE usuarios = ?";
    ...
    if($result->num_rows == 0)
    {
      ...
      $sql = "INSERT INTO proyecto2023_emma (usuarios, contrasenia) VALUES (?, ?)";
      $statement = $conn->prepare($sql);
...
if(isset($_POST["submit"]))
{
  ...
  $sql = "SELECT * FROM proyecto2023_emma WHERE usuarios = ? AND contrasenia = ?";
  $statement = $conn->prepare($sql);
  $statement->bind_param("ss", $username, $password);
  $statement->execute();
  $result = $statement->get_result();
